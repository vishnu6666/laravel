...
class becupTipsController extends Controller
{
    /**
    ...
    public function getUpcomingAndLatestGameData()
    {
        ...
        $todayDate = Carbon::now()->format('Y-m-d');
            return Game::selectRaw('max(games.id) as id,max(games.gameName) as gameName, 
                                max(games.gameFullName) as gameFullName,
                                IF(MAX(user_game_history.isSubscribed = 1),1,0) as isSubscribed,
                                IF(MAX(user_game_history.isKeepNotification = 1),1,0) as isKeepNotification,
                                IF(games.launchDate = "'.$todayDate.'",1,0) as isLatest,
                                CONCAT("' . $gameImagePath . '/",max(games.gameImage)) as gameImage, games.id,
                                COUNT(DISTINCT(user_game_history.userId)) AS totalUsersCount,
                                COUNT(DISTINCT(games_tips.id)) AS totalTips,
                                max(games_tips.createdAt) as tipDate,
                                ROUND(AVG(games_tips.accuracyRate), 2) as percentage,
    ...
    public function getspecificGameData($search = '', $date = '')
    {
        ...
        $data = \DB::select('select games.id,games.gameName,games.gameFullName,
                                        games.gameImage,CONCAT("' . $gameImagePath . '/",max(games.gameImage)) as gameImage,
                                        count(DISTINCT(user_game_history.userId)) as totalUsersCount,
                                        IF(MAX(`ugh`.`isKeepNotification` = 1),1,0) as isKeepNotification,
                                        count(DISTINCT(games_tips.id)) as totalTips,
                                        '.$select.'
                                        IF(COUNT(DISTINCT(user_messages.id)) > 0, 1, 0) as isLatest,
                                        games.accuracyRates as percentage,
                                        IF(MAX(user_game_history.userId = "'.$authUser->id.'"),1,0) as isSubscribed from `games` 
    ...
    /*
       return Game::selectRaw('games.id,games.gameName,games.gameFullName,
                                        games.gameImage,CONCAT("' . $gameImagePath . '/",max(games.gameImage)) as gameImage,
                                        count(DISTINCT(user_game_history.userId)) as totalUsersCount,
                                        IF(MAX(`ugh`.`isKeepNotification` = 1),1,0) as isKeepNotification,
                                        count(DISTINCT(games_tips.id)) as totalTips,
                                        IF(COUNT(DISTINCT(user_messages.id)) > 0, 1, 0) as isLatest,
                                        games.accuracyRates as percentage,
                                        IF(MAX(user_game_history.userId = "'.$authUser->id.'"),1,0) as isSubscribed')
                                        ->whereNull('games_tips.deletedAt')
        ->join('user_game_history', function ($join) {
            $join->on('games.id', '=', 'user_game_history.gameId')->where('isActive','1');
            
        })
        ->leftJoin('user_game_history as ugh', function ($join) {
            $join->on('games.id', '=', 'ugh.gameId')->where('isActive','1');
            
        })
        
        ->leftJoin('user_messages', function ($join) use($authUser){
            $join->on('user_messages.gameId', '=', 'games.id')
                 ->where('user_messages.isRead', 0)
                 ->where('user_messages.userId', $authUser->id);     
        })
        ->leftJoin('games_tips', function ($join) {
            $join->on('games_tips.gameId', '=', 'games.id');
        })
    ...
    public function getTipswithFilter(Request $request)
    {
        ...
        else
        {
            $gameTipsDetail = collect($data)->sortByDesc('id')->values();
                ...
                /*$gameTipsDetail = Game::selectRaw('max(games.id) as id,ma x(games.gameName) as gameName, 
                max(games.gameFullName) as gameFullName, max(games.isActive) as isActive1,
                IF(MAX(user_game_history.isSubscribed = 1),1,0) as isSubscribed,
                IF(MAX(user_game_history.isKeepNotification = 1),1,0) as isKeepNotification,
                CONCAT("' . $gameImagePath . '/",max(games.gameImage)) as gameImage, games.id ,
                COUNT(DISTINCT(user_game_history.userId)) AS totalUsersCount,
                IF(games_tips.createdAt = "'.$todayDate.'",1,0) as isLatest,
                COUNT(DISTINCT(games_tips.id)) AS totalTips,ROUND(AVG(games_tips.accuracyRate), 2) as percentage')
                ->having('isActive1',1)

                ->$joinType('user_game_history', function ($join) use($request,$authUser) {
                        $join->on('games.id', 'user_game_history.gameId')
                            ->where('user_game_history.userId', $authUser->id);
                })

                ->Join('games_tips', function ($join) use($request) {
                    $join->on('games_tips.gameId', '=', 'games.id');

                    if($request->filter =='today'){
                        $join->whereRaw('DATE(games_tips.createdAt) = "'.Carbon::now()->format('Y-m-d').'"');
                    }

                    if($request->filter =='yesterday'){
                        $join->whereRaw('DATE(games_tips.createdAt) = "'.Carbon::yesterday()->toDateString().'"');
                    }
                })
                ->leftJoin('packages_asign_to_game', function ($join) use($request) {
                    $join->on('packages_asign_to_game.gameId', '=', 'games.id');
                })
                ->leftJoin('packages', function ($join) use($request) {
                    $join->on('packages.id', '=', 'packages_asign_to_game.packageId');
                });
                $gameTipsDetail  =$gameTipsDetail->groupBy('games.id');

                if($request->filter =='popular'){
                    $gameTipsDetail = $gameTipsDetail->orderBy('totalUsersCount','desc');
                }else{
                    $gameTipsDetail = $gameTipsDetail->orderBy('games.id','desc');
                } 
            
                $gameTipsDetail->where('packages.packageName','LIKE','%'.$request->searchSport.'%');
                $gameTipsDetail->orWhere('games.gameName','LIKE','%'.$request->searchSport.'%');
                
                $gameTipsDetail = $gameTipsDetail->get();
                //echo "<pre>";print_r($gameTipsDetail);exit;*/
        }
    ...
    public function getMysport(Request $request)
    {
        ...
        $todayDate = Carbon::now()->format('Y-m-d');
            ...
            ->with(['games'=> function($qry) use($gameImagePath,$todayDate, $authUser){
                ...
                games.gameFullName,games.gameImage,
                CONCAT("' . $gameImagePath . '/",max(games.gameImage)) as gameImage,
                count(DISTINCT(user_game_history.userId)) as totalUsersCount,
                IF(MAX(user_game_history.isSubscribed = 1),1,0) as isSubscribed,
                IF(MAX(ugh.isKeepNotification = 1),1,0) as isKeepNotification,
                count(DISTINCT(games_tips.id)) as totalTips, 
        ...
        if($searchPackage){             
            $searchSportDetails = Package::selectRaw('id,packageName')
                                ->with(['games'=> function($qry) use($gameImagePath){
                                    ...
                                    games.gameImage,CONCAT("' . $gameImagePath . '/",max(games.gameImage)) as gameImage,
                                    count(DISTINCT(user_game_history.id)) as totalUsersCount,
                                    IF(MAX(user_game_history.isSubscribed = 1),1,0) as isSubscribed,
                                    IF(MAX(user_game_history.isKeepNotification = 1),1,0) as isKeepNotification,
                                    count(DISTINCT(games_tips.id)) as totalTips, 
                                    games.accuracyRates as percentage')
    ...
    public function getCompareSport(Request $request)
    {
        ...
        $todayDate = Carbon::now()->format('Y-m-d');
            ...
            ->with(['games'=> function($qry) use($gameImagePath,$todayDate){
                ...
                CONCAT("' . $gameImagePath . '/",max(games.gameImage)) as gameImage,
                count(DISTINCT(user_game_history.userId)) as totalUsersCount,
                count(DISTINCT(gt.id)) as totalTips, 
                IF(games.launchDate = "'.$todayDate.'",1,0) as isLatest,
                games.accuracyRates as percentage,
                CONCAT(games_tips.accuracyRate," %") AS accuracyRate')
                ->leftJoin('games', function ($join) {
                    $join->on('games.id', '=', 'packages_asign_to_game.gameId');
                })
                ->leftJoin('games_tips', function ($join) {
