...
class AdminResetPasswordController extends ResetPasswordController
{
    ...
    */
    ...
    {  
        ...
        return view('admin.auth.reset_password', ['email' => $request->email, 'token' => $token, 'alreadySet' => $alreadySet,'message' => $message]);
    }
        ...
        if(!empty($user))
        {
            ...
            if(!empty($record)){
                ...
                if($user->save())
                {
                    ...
                    // return view('admin.auth.reset_password', ['message' => $message, 'email' => $request->email, 'api' => 0, 'alreadySet' => 3]);
                }
            }else{
                $message = "Password Already Reset.";
                return  redirect(route('adminLogin'))->with('success', $message);
                // return redirect('adminLogin')->with('success', $message);
                // return view('admin.auth.reset_password', ['message' => $message, 'email' => $request->email, 'api' => 0, 'alreadySet' => 1]);
            }
            
        }else{
            $message = "Email is not available in system.";
            return view('admin.auth.reset_password', ['message' => $message, 'email' => $request->email, 'api' => 0, 'alreadySet' => 2]);
        }
    }
}
