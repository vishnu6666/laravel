...
class becupUserCardsController extends Controller
{
    /**
     * Get user card
     * 
    ...
    public function getUserCards(Request $request)
    {
        ...
        if(!empty($user))
        {
            try 
            {
                ...
                if(count($cardList) >= 1){
                    return $this->toJson(['cards' => $cardList], trans('api.user.card_list'),1);
                }
                return $this->toJson([], trans('api.user.card_not_found'), 0);
            }
            catch (\Exception $e) {
                    ...
                    try {
                        ...
                        if($deleteCard->deleted)
                        {
                            ...
                            if($request['cardId'] == $user->cardToken){
                                $user->cardToken = $cardList[0]['cardToken'];
                                $user->save();
                            }
